...
选中文字 → `Ctrl+C` 复制 → `Ctrl+Enter` → 侧栏出卡片。全程不离开编辑器，不切换 App，不丢失上下文。
...
```text
选中文字  →  Ctrl+C  →  Ctrl+Enter  →  侧栏查看 AI 解释  →  追问  →  导出知识
```

> 在 Claude Code 回答窗口中选中文字后，先 `Ctrl+C` 复制，再按 `Ctrl+Enter` 触发（VSCode API 限制，无法直接读取 webview 选区）。
...
Select any text → `Ctrl+C` → `Ctrl+Enter` → A card appears in the sidebar with a context-aware explanation powered by your full Claude Code conversation history. Follow up with questions inside the card, then export everything as a structured knowledge document.
...
5. Select text → `Ctrl+C` → `Ctrl+Enter` → Done
