...
在 VSCode 扩展面板搜索 **AskLens** 安装，或从 [Releases](../../releases) 下载 `.vsix` 手动安装。

### 配置

1. 打开命令面板 `Ctrl+Shift+P`，执行 **AskLens: 切换 Provider / 模型** 选择 AI 服务
2. 执行 **AskLens: 设置 API key** 输入对应 Key（Ollama 用户跳过）
...
2. Run `AskLens: Switch Provider / Model` to select your AI service
3. Run `AskLens: Set API Key` to enter your key
4. Install the SessionStart Hook when prompted (recommended)
5. Select text → `Ctrl+Enter` → Done
